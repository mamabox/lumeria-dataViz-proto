"""
Video exporter — combines animated map + gameplay video into a single mp4.

Renders plotly animation frames side by side with gameplay footage,
producing one video file that stays perfectly in sync.

Approach: saves each combined frame as a PNG in a temp folder,
then assembles them into a video with a single ffmpeg command.
...
def export_combined_video(
    ...
    and the animated map on the right.
    ...
    # --- Phase 1: Save frames as PNGs ---
    frames_dir = tempfile.mkdtemp(prefix="lumeria_export_")
    ...
    # --- Phase 2: Assemble video with ffmpeg (no pipes) ---
    subprocess.run(
        [
            "ffmpeg", "-y",
